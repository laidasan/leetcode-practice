...
let nums = [1,0,3,0,21]
//不符合題目要求，需要一樣的順序，只是0移動到後面
// function moveZeros(nums) {
//     nums.sort((a , b) => a - b)
//     while(nums[0] === 0 && !nums.every(nm => nm === 0)) {
//         nums.push(nums.splice(0,1)[0])
//     }
// }
// moveZeros(nums)
// console.log(nums)
// console.log([1,100,5,20,67,3].sort((a,b) => a - b))


function moveZeros(nums) {
    let index = 0
    let zeros = []
    while(index < nums.length && !nums.every(nm => nm === 0)) {
        while(nums[index] === 0) {
            zeros.push(nums.splice(index,1)[0])
        }
        index++;
    }
    zeros.forEach(zero => nums.push(zero))
}
